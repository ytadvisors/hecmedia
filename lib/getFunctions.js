...
import { HOME_FEED_MOBILE_MAX_WIDTH } from "./homeFeedResponsive";
import {
  isExternalNavUrl,
  isSiteRelativePath,
  menuNodeToNavUrl,
...
export const getPostImgSizes = layout => {
  const mobileSize = `(max-width: ${HOME_FEED_MOBILE_MAX_WIDTH}px) 100vw`;
  if (layout === "Featured" || layout === "Wallpaper") {
    return `${mobileSize}, 768px`;
  }
  if (layout === "3 Columns" || layout === "3-columns") {
    return `${mobileSize}, 33vw`;
  }
  return `${mobileSize}, 50vw`;
};
