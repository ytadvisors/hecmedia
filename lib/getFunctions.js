...
export const getHeaderMenuObject = (menus, isRootLevel = true) =>
  menus
    ? menus
        ...
        .map(link => {
          const { node } = link;
          const relativeUrl = menuNodeToRelativeUrl(node);
          const childEdges =
          ...
          return {
            label: node.label,
            // Always site-relative so Next routing stays on localhost / CDN host.
            url: relativeUrl,
  ...
  menus
    ? menus
        ...
        .map(link => ({
          ...
          // Social destinations are usually external; only rewrite known WP hosts.
          // The canonical URL must win over a GraphQL/REST path so external
          // social destinations cannot be collapsed to site-relative routes.
          link: toSiteRelativeUrl(link.node.url || link.node.path),
