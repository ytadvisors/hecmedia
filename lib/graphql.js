...
/**
 * Staging presentation controls are deployed independently from the legacy
 * shell schema. Keep this query isolated so an older production schema cannot
...
export const GET_CURATED_TRENDING_POSTS = gql`
  query CuratedTrendingPosts($ids: [ID]) {
    curatedTrendingPosts: posts(first: 4, where: { in: $ids }) {
      nodes {
        title(format: RENDERED)
        postDetails {
          videoImage {
