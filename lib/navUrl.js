 ...
 *
 * Safe external absolute URLs (http/https/mailto/tel) → keep full href and open
 * externally. Unsafe schemes (javascript:, data:, vbscript:, …) are rejected so
 * CMS-controlled menu/CTA values cannot inject executable hrefs into the app.
 */
...
/** Absolute schemes allowed in rendered hrefs (CMS / menu controlled). */
const SAFE_ABSOLUTE_SCHEMES = new Set(["http:", "https:", "mailto:", "tel:"]);

/**
 * @param {string|null|undefined} protocol e.g. "https:" from URL.protocol
 * @returns {boolean}
 */
export const isSafeAbsoluteScheme = protocol =>
  SAFE_ABSOLUTE_SCHEMES.has(String(protocol || "").toLowerCase());
...
export const isExternalNavUrl = urlOrPath => {
  ...
  try {
    const u = new URL(raw);
    // Only safe absolute schemes can be external destinations.
    if (!isSafeAbsoluteScheme(u.protocol)) return false;
...
export const resolveNavUrl = urlOrPath => {
  ...
  try {
    const u = new URL(raw);
    if (!isSafeAbsoluteScheme(u.protocol)) {
      // Reject javascript:, data:, vbscript:, and any other non-allowlisted scheme.
    ...
    // Never promote scheme-like junk (e.g. "javascript:…") into a path href.
