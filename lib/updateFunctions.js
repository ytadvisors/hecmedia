...
export const getQueryUpdate = (prev, fetchData, type) => {
  const { fetchMoreResult } = fetchData;
  const results = { ...fetchMoreResult };
  if (!fetchMoreResult) {
    return prev;
  }
  if (prev && prev[type] && prev[type].nodes.length > 0) {
    results[type].nodes = [...prev[type].nodes, ...results[type].nodes];
  }
  return results;
};
