...
describe("legacy WordPress post filters", () => {
...
describe("GET_NEWEST_VIDEOS legacy media compatibility", () => {
  it("requests the direct MediaItem shape used by upstream WordPress", () => {
    const query = print(GET_NEWEST_VIDEOS);

    expect(query).toMatch(/featuredImage\s*\{\s*sourceUrl/);
    expect(query).not.toMatch(/featuredImage\s*\{\s*node/);
