...
describe("deprecated magazine and event collections", () => {
  it("keeps deprecated collection fields out of the site shell", () => {
    const query = print(GET_LAYOUT);

    expect(query).not.toContain("magazines");
...
describe("GET_PAGE_INFO postHero", () => {
  it("requests postHero for the main post page only", () => {
    ...
    const { GET_PAGE_INFO } = require("./graphql");
    const query = print(GET_PAGE_INFO);
