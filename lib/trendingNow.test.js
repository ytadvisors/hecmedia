...
describe("toTrendingNowItems", () => {
  ...
  it("caps the deterministic curated-plus-newest list at four", () => {
    const post = postId => ({
      postId,
      title: `Video ${postId}`,
    ...
    expect(
      toTrendingNowItems([post(1), post(2)], [post(3), post(4), post(5)])
    ).toHaveLength(4);
    expect(
      toTrendingNowItems([post(1), post(2)], [post(3), post(4), post(5)]).map(
        item => item.id
      )
    ).toEqual([1, 2, 3, 4]);
    expect(
      toTrendingNowItems([post(1), post(2)], [post(3), post(4), post(5)], 12)
    ).toHaveLength(4);
  });
});
