 ...
 */
export const normalizeForEducators = value => {
  const raw = value || {};
...
export const mergeHecSiteChrome = (siteSettings, legacySiteContent) => {
  ...
  const hasCanonicalEducators = Boolean(
    siteSettings && siteSettings.forEducators
  );
  const hasMax =
    siteSettings &&
  ...
  const forEducators = hasCanonicalEducators
    ? normalizeForEducators(siteSettings.forEducators)
    : legacy.forEducators;
