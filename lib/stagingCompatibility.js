...
export const DEFAULT_SITE_CONTENT = {
  ...
  // Content stays first unless the CMS explicitly promotes the rail on mobile.
...
export const normalizeSiteContent = value => {
  ...
  return {
    ...
    maxVideos,
    trendingTitle:
      (typeof content.trendingTitle === "string" &&
        content.trendingTitle.trim()) ||
      DEFAULT_SITE_CONTENT.trendingTitle,
    spotlightTitle:
      (typeof content.spotlightTitle === "string" &&
        content.spotlightTitle.trim()) ||
      DEFAULT_SITE_CONTENT.spotlightTitle,
    mobileRailFirst: content.mobileRailFirst === true,
    footerLinks:
...
/**
 * Map CMS mobileDisplay onto mobileRailFirst, or keep the prior value when
 * the field is unset. Content stays first unless the CMS explicitly chooses
 * menu-content.
 */
export const resolveMobileRailFirst = (
  mobileDisplay,
  legacyMobileRailFirst
) => {
  if (mobileDisplay === "menu-content") return true;
  if (mobileDisplay === "content-menu") return false;
  return legacyMobileRailFirst === true;
};
...
export const mergeHecSiteChrome = (siteSettings, legacySiteContent) => {
  ...
  const trendingSettings =
    (siteSettings && siteSettings.trendingSettings) || {};
  const hasMax = siteSettings && trendingSettings.maxVideos != null;
  ...
  const maxVideos = hasMax
    ? normalizeMaxVideos(trendingSettings)
    : legacy.maxVideos;

  const trendingTitle =
    (typeof trendingSettings.trendingTitle === "string" &&
      trendingSettings.trendingTitle.trim()) ||
    legacy.trendingTitle;
  const spotlightTitle =
    (typeof trendingSettings.spotlightTitle === "string" &&
      trendingSettings.spotlightTitle.trim()) ||
    legacy.spotlightTitle;

  // mobileDisplay is optional on the GraphQL contract. When absent/null
  // (isolated presentation query failed, or pre-WP#52 schema), keep the
  // longstanding content-first default from legacy/DEFAULT_SITE_CONTENT.
  // Only an explicit content-menu | menu-content value may flip the rail.
  // WP must also default unsaved Mobile Display to content-menu so an
  // always-present GraphQL field cannot promote the rail without an editor.
  const mobileRailFirst = resolveMobileRailFirst(
    trendingSettings.mobileDisplay,
    legacy.mobileRailFirst
  );
