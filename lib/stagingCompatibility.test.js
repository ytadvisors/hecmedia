...
describe("legacy staging CMS compatibility", () => {
  ...
  it("merges HEC Site Settings over legacy hectvSiteContent for the rail", () => {
    ...
    expect(chrome.mobileRailFirst).toBe(true);
    expect(chrome.trendingTitle).toBe("Trending Now");
    expect(chrome.spotlightTitle).toBe("Spotlight STL");
  ...
  it("keeps content first unless the CMS explicitly promotes the rail", () => {
    expect(mergeHecSiteChrome(undefined, undefined).mobileRailFirst).toBe(
      false
    );
    expect(
      mergeHecSiteChrome(undefined, { mobileRailFirst: false }).mobileRailFirst
    ).toBe(false);
    expect(
      mergeHecSiteChrome(undefined, { mobileRailFirst: true }).mobileRailFirst
    ).toBe(true);

    // Post-upgrade / partial presentation: absent or null mobileDisplay must
    // not flip the longstanding content-first default.
    expect(
      mergeHecSiteChrome(
        { trendingSettings: { maxVideos: 5 } },
    ...
    expect(
      mergeHecSiteChrome(
        { trendingSettings: { mobileDisplay: null } },
        undefined
      ).mobileRailFirst
    ).toBe(false);
    expect(
  ...
  it("uses canonical rail headings and mobile display over legacy content", () => {
    const contentFirst = mergeHecSiteChrome(
      {
        trendingSettings: {
      ...
      {
        spotlightTitle: "Legacy Spotlight",
        mobileRailFirst: true
      }
    );

    expect(contentFirst.trendingTitle).toBe("Popular Today");
    expect(contentFirst.spotlightTitle).toBe("Around St. Louis");
    expect(contentFirst.mobileRailFirst).toBe(false);

    expect(
      mergeHecSiteChrome(
        { trendingSettings: { mobileDisplay: "menu-content" } },
        { mobileRailFirst: false }
      ).mobileRailFirst
    ).toBe(true);
  });
