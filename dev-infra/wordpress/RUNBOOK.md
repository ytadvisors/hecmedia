...
compose file binds every port to `127.0.0.1` on worker-mba only (`docker port hecmedia-dev-wp` confirms `80/tcp -> 127.0.0.1:8091`, not `0.0.0.0`), so
...
### Temporary staging over Tailscale Funnel

`funnel-router.js` supports a staging-only hybrid endpoint when production
WordPress cannot yet expose newly developed optional fields. It routes only
`RailPromo`, `TopbarCtas`, `FeaturedVideos`, and `PostHeaderImageSize` GraphQL
operations, plus the local site-options REST fixture, to the local Docker
WordPress. All established GraphQL and REST reads continue to
`https://prod-wp.hectv.org`.

Run the router behind an SSH tunnel to this Docker service and expose only
`/graphql`, `/wp-json`, and `/wp-content/uploads` through Funnel. Never expose
the WordPress root, `/wp-admin`, or `/wp-login.php`. This is a temporary staging
QA topology, not a production backend.
