...
| Analytics pollution                      | Request interception is installed before navigation with cache and service workers disabled; only GET/HEAD requests for exact page/static paths, WordPress uploads, the reviewed HEC asset-library prefix, and the exact loader are allowed. HTTP, WebSocket, and two-server redirect denial are integration-tested; every other destination is blocked and recorded without interaction.                                             |
| Loader verification TOCTOU               | Chrome's exact loader request is intercepted and fetched with `maxRedirects: 0`; only exact HTTP 200, the unchanged exact loader URL, JavaScript content type, and approved version/canonical hash/counts/inventory are fulfilled. A redirect is aborted before its destination receives traffic.                                                                                                                                     |
...
| S3 overwrite not rolled back             | Every existing collision is downloaded and SHA-256 verified, copied with source-ETag and destination-absence preconditions to a task/run/attempt/release-scoped preimage, downloaded and verified again, and bound in a conditionally created manifest before candidate writes                                                                                                                                                        |
...
| Candidate changes after preimage capture | The complete local asset tree key/size/SHA-256/content-type inventory is sealed and revalidated. Manifest-driven candidate PUTs atomically require the captured ETag for collisions or absence for additive keys; every uploaded byte is verified before Lambda/CloudFront mutation.                                                                                                                                                  |
...
| Broken immutable tag                     | A tested controller preflights an absent or exact annotated tag before AWS and verifies the peeled commit as the final operation in a GitHub-write-only job. Success requires both a green terminal tag job and the exact annotated tag. Automatic deletion is forbidden because GitHub ref deletion has no compare-and-swap precondition; any non-success tag job restores production and preserves the tag for reviewed resolution. |
| Build dependencies holding AWS authority | Installation, tests, package build, Chrome, and public verification run in contents-read jobs with no OIDC. AWS mutation/recovery jobs have no contents-write permission and execute no browser runtime; the terminal tag job has contents-write but no AWS authority.                                                                                                                                                                |
| Cancellation, timeout, or runner loss    | An independent `always()` AWS recovery watchdog arms through an immutable S3 handshake before the candidate write fence, monitors mutation/public-verification/tag jobs, and restores the bound baseline for any non-success terminal path. The mutator also retains immediate in-process recovery for ordinary errors.                                                                                                               |
...
Acceptance requires one exact HTTP 200 response with zero redirects and an unchanged final loader
URL, one bootstrap, one initial `gtm.js` event, and zero successful non-allowlisted or redirect-
destination requests. Passive analytics-owner observation is
...
3. For an existing key, download the current object, capture content metadata, ETag, and VersionId
   as evidence, and conditionally copy it (source ETag must match; destination must be absent) to
...
5. Conditionally create and read back the manifest; record its exact key and SHA-256 in workflow
   evidence and the living log.
6. Revalidate the sealed tree and remote baseline, arm the independent recovery watchdog, set the
   immutable candidate-write fence, then upload each manifest key with `If-Match` for collisions or
   `If-None-Match: *` for additive keys. There is no unconditional `aws s3 sync`.
7. Download and hash every uploaded candidate object before publishing Lambda or changing
   CloudFront.
8. After any later failure/cancellation/timeout/runner loss, classify each collision as exact
   baseline, exact candidate, or unknown. Restore only the exact candidate with source/destination
   ETag preconditions; unknown drift fails closed and suppresses invalidation. Then restore
   Lambda/CloudFront if touched and verify the sanitized public site.
