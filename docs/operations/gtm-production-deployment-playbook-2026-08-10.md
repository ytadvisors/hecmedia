...
Plan PR #272 merged as `d5431eeff9c98530a7414bc39eb63b590693627a`. The later owner instruction
to proceed authorizes the separately reviewed remediation workflow, not a production dispatch while
any ledger gate is NO-GO. The
[deployment audit](gtm-production-deployment-audit-2026-08-10.md) is the durable rationale and
repeatable evidence companion to this playbook.
...
The selected B5 path republishes only the existing frontend Lambda, syncs frontend assets without
`--delete`, and updates the existing CloudFront distribution while keeping the newsletter API
behavior absent. The release is therefore a governed full frontend publish with a GTM objective,
not a one-line GTM mutation.
...
automatically restored version `150`. The current live page is HTTP 200, has a valid `HEC on YouTube` title and release metadata, but correctly does not contain that literal. Current `master`
...
step had no Git committer identity. The planning anchor still calls `git tag -a` without a
...
The remediation implementation selects path 2: general forms remain send-enabled, while targeted
newsletter omit mode prevents publication/association of the newsletter API and renders the
newsletter signup unavailable. Exact-head PR approval is the final ratification of that code path.
...
response's headers, raw byte length/SHA-256, and tag destinations as secondary transport evidence,
then freeze GTM publishing through the deployment/soak window. Raw `gtm.js` bytes are not an
equality gate: identical fixed requests produced different runtime bytes while `data.resource`
remained unchanged. The reviewed verifier must gate on `canonical-resource-v1`—exactly one parsed
`var data` object, recursively sorted object keys, preserved array order, JSON scalar encoding—plus
the public resource version, canonical SHA-256, counts, and normalized inventory. Any approved
semantic version/hash/inventory drift restarts review.
The governed owner record must checksum a regular, in-repository GTM export-format-v2 JSON artifact
whose `container.publicId` is exactly `GTM-57RZPNN` and whose container-version ID is the approved
public resource version. Its decision list is exhaustive and fixed to the eight reviewed surfaces;
removing a row or approving only a subset fails closed.
The owner decision is recorded in the SHA-bound
[`gtm-owner-approval-2026-08-10.json`](gtm-owner-approval-2026-08-10.json); `PENDING` is not
approval.
...
2. Clear B1–B11 through narrowly scoped reviewed changes, configuration evidence, and the explicit
...
| `expected_gtm_resource_version`       | Analytics-owner-approved published resource version          |
| `expected_gtm_canonical_sha256`       | Approved `canonical-resource-v1` SHA-256                     |
| `expected_gtm_counts`                 | Approved `macros/tags/predicates/rules` counts               |
| `expected_gtm_inventory_sha256`       | Approved `gtm-normalized-inventory-v1` SHA-256               |
| `gtm_owner_approval_sha256`           | SHA-256 of the reviewed in-repo owner/export/freeze decision |
| `decommission_closeout_sha256`        | SHA-256 of the reviewed in-repo observation closeout         |
...
- [ ] A normal response plus two cache-busted fresh-origin checks on both production hostnames and
      hydrated-browser checks agree with the expected candidate.
...
- [ ] The post-cutover GTM resource version, `canonical-resource-v1` SHA-256, counts, and normalized
      tag/trigger/destination inventory match the analytics-owner-approved pre-cutover evidence.
- [ ] Post-cutover raw byte length/SHA-256/headers are recorded as `secondary_non_gate`; raw hash
      variance alone is neither GO nor rollback.
...
- [ ] If B5 selected omit: no newsletter API version is published or associated, the behavior
      remains absent, general forms remain send-enabled, and the targeted newsletter-unavailable
      frontend/verifier contract passes.
...
- explicit rollback-only inputs for the original deploy task ID, deploy run ID, deploy run attempt,
  original release SHA, baseline release SHA/ETag, exact preimage manifest S3 key, and manifest
  SHA-256;
- fresh audit inputs from the live distribution; and
- confirmation `ROLLBACK HEC FRONTEND PRODUCTION`.

The rollback-only selectors and fresh current ETag/default Lambda ARN/checksum/API inputs must all
resolve to the exact original deployment and prove that candidate is still the affected live state
before any S3 copy or CloudFront update.
...
`/Users/ytwguru/.openclaw/workspace-root/deliverables/hecmedia/gtm-production-2026-08-10/<attempt-id>/`

If a different approved commander executes the attempt, the living log must replace this path with
that commander's fully expanded absolute workspace path and record the `ls -la` proof before
authorization.
...
- original deploy task/run/attempt and the exact preimage manifest S3 key/SHA-256 binding;
- local and Actions test/build/verifier outputs;
- queue-task receipts, run URL/ID, environment approval, and release tag proof;
- raw HTML, `canonical-resource-v1`, normalized inventory, and browser GTM/network/console evidence
  with analytics collection controls noted;
...
| B1–B11 remediation/decision | Separately approved PR URL(s), CI/config/S3/GTM/backend evidence, newsletter decision, merged SHA(s) | Pending |
